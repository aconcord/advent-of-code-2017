...
/**
 * Part One:
 * Find the difference between min/max of each row.
 * Add all the differences up.
 */

function differenceMinMax(line /* Number[] */) {
...
const partOneSolution = input.map(line => differenceMinMax(line))
    .reduce(sum, 0)
 ...
 */
 ...
 function divideEvenlyDivisbleNumbers(line /* Number[] */){
    for (let i = 0; i < line.length; i++) {
        const arrWithoutIndex = line.filter((ele, index) => index !== i);
        const curr = line[i];
        
        for (let j = 0; j < arrWithoutIndex.length; j++) {
            if (curr % arrWithoutIndex[j] === 0) {
                return curr / arrWithoutIndex[j];
            }
        }
    }
    throw new Error('Compared every number to every other number and didn\'t find any that evenly divide each other');
 }

const partTwoSolution = input.map(line => divideEvenlyDivisbleNumbers(line))
    .reduce(sum, 0);
